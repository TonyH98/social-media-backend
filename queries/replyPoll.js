...
const getReplies = async (pollId) => {
    try{
        const allReplies = await db.any(
            `SELECT r.id, r.poll_id, r.content, r.gif, to_char(r.date_created, 'MM/DD/YYYY') AS time, 
        ...
        addPost = await db.tx(async (t) => {
          ...
          if(articleUrlMatch){
            ...
            const insertedPost = await t.one(
              'INSERT INTO replies (poll_id, user_id, content, posts_img, gif, url, url_img, url_title) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING *',
              [post.poll_id , post.user_id, postContent, post.posts_img, post.gif, articleUrl, articleImage, articleTitle]
            );
            ...
            const insertedPost = await t.one(
              'INSERT INTO replies (poll_id, user_id, content, posts_img, gif, url, url_img, url_title) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING *',
              [post.poll_id , post.user_id, post.content, post.posts_img, post.gif, null , null , null]
            );
